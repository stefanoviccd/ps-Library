...
public class RentBookSO extends AbstractSO {
    ...
    private int checkBookCount(Book b) throws Exception {
        GetBooksByQuerySO getBooksByQuerySO = new GetBooksByQuerySO();
        try {
            String query = "SELECT * FROM knjiga WHERE id=" + b.getBookid();
            Book book = ((List<Book>) getBooksByQuerySO.execute(query)).get(0);
